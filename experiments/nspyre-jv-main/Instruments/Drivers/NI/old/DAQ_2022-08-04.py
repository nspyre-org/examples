...
# @Date:   2022-03-25T11:15:06-07:00
# @Email:  ericros@stanford.edu
# @Project: nspyre-jv
# @Last modified by:   Eric Rosenthal
# @Last modified time: 2022-07-16T15:49:51-07:00
...
class DAQ():
    ...
    def initialize(self):
        ...
        points = round(self.DAQ_PARAMS['sampling_rate']*self.DAQ_PARAMS['time_per_point'])

        # check to make sure buffer size is large enough
        if points < 2:
            print('Warning: buffer size must be greater than 2.')
            points = 2
            self.DAQ_PARAMS['sampling_rate'] = int(points/self.DAQ_PARAMS['time_per_point'])
            print('Changing sampling rate to ' + str(self.DAQ_PARAMS['sampling_rate']) + ' so that buffer size = 2.')

        self.buffer_size = int(points)
        # self.buffer_size = self.DAQ_PARAMS['buffer_size'] #int(points)

        self.real_point_time = (points-1)/self.DAQ_PARAMS['sampling_rate']
        ...
        clk_channel = self.DAQ_PARAMS['dev_name'] + '/' + 'port0'
        ...
        for ch_idx, key in enumerate(self.DAQ_PARAMS['channels']):
            dev_channel = self.DAQ_PARAMS['dev_name'] + '/' + self.DAQ_PARAMS['channels'][key]['ctr_name'] #channel
        ...
        for ch_idx, key in enumerate(self.DAQ_PARAMS['channels']):
            ...
            pfi_channel = '/' + self.DAQ_PARAMS['dev_name'] + '/' + self.DAQ_PARAMS['channels'][key]['pfi_name']
        ...
        self.n_chan = self.DAQ_PARAMS['channels'].__len__()
    ...
    def set_ao_voltage(self, v, ch=0):
        ch_name = self.DAQ_PARAMS['dev_name'] + '/' + 'ao' + str(ch)
    ...
    def read_ai_voltage(self, ch=0):
        ch_name = self.DAQ_PARAMS['dev_name'] + '/' + 'ai' + str(ch)
        with nidaqmx.Task() as task:
