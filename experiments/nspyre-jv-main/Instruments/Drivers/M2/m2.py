...
# @Date:   2022-07-13T15:18:00-07:00
# @Email:  ericros@stanford.edu
# @Project: nspyre-jv
# @Last modified by:   Eric Rosenthal
# @Last modified time: 2022-07-22T10:43:26-07:00


from pylablib.core.utils import net, general
from pylablib.core.devio import interface, comm_backend
from pylablib.core.devio.comm_backend import reraise
...
class Solstis(interface.IDevice):
    ...
    def _recv_reply(self, expected_report=None):
        while True:
            ...
            print('testing _recv_reply....')
            print(reply)

            preply=self._parse_reply(reply)

            print('testing _recv_reply2....')
            print(preply)
    ...
    def query(self, op, params, reply_op="auto", report=False):
        ...
        try:
            ...
            # print('no preply received')
            preply = 'none'

        if reply_op=="auto":
            reply_op=op+"_reply"
        if reply_op and preply[0]!=reply_op:
            print("warning: unexpected reply op: '{}' (expected '{}')".format(preply[0],reply_op))
    ...
    def tune_resonator(self, value, fine=False, sync=True):
        op_name="fine_tune_resonator" if fine else "tune_resonator"
        # self.write(op_name,{"setting":[value]})
        self.query(op_name,{"setting":[value]})

    def query_resonator(self, value=0, fine=False, sync=True):
        op_name="fine_tune_resonator" if fine else "tune_resonator"
        # query_reply = self.query(op_name,{"status":[value]})
        query_reply = self.query(op_name,{"reply":[value]})
    ...
    def tune_laser_resonator_legacy(self, value, fine=False, sync=True):
        """
        ...
        # msg=self._build_message(op_name,value)
        # self.socket.send(msg)

        # {'message': {'transmission_id': [1], 'op': 'start_link'}}

        # msg = {'message': {'transmission_id': [1], 'op': 'start_link'}}
    ...
    def start_fast_scan(self, scan_type, width, period):
        self.write("fast_scan_start",{"scan":scan_type,"width":[width/1E9],"time":[period]})
    ...
    def stop_fast_scan(self, scan_type, return_to_start=True, sync=False):
        ...
        self.write(op_name,{"scan":scan_type})

    def start_fast_scan_legacy(self, scan_type, width, period, sync=False, setup_locks=True):
        """
    ...
    def stop_fast_scan_legacy(self, scan_type, return_to_start=True, sync=False):
        """
