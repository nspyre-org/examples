...
def _main():
    ...
    for sp in setpoints:
        ...
        _log(f'Moving to setpoint of {target_temperature:.3f} K with rate of {rate:.3f} K/min and target stability of {target_stability:.3f} K.')
        ...
        while True:
            ...
            while True:
                ...
                # Wait for temperature to stabilize at current setpoint before moving to next
                stability_ok, is_stable = cryo.get_platform_temperature_stable()
                if is_stable:
                    break
                else:
                    _log(f'  Waiting for platform to stabilize at intermediate setpoint, target stability is {target_stability:.3f} K.')

        #
        # Reached desired setpoint temperature.  Wait for stable
        # temperature, and then wait a period for conducting
